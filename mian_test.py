...
for i in load_data_from_orcale():
    ...
    if model_one_temp == 0:
        Ins2Orc.insert_(state=0, b_r='', result_dict=init_result_dict, DEVICECODE=DEVICECODE,
                        result_cp=result_cp, end_result_cp=init_end_result, broken_reason='')
    ...
    param_2 = [(DEVICECODE, datetime.datetime.now(), float(end_result_cp.iloc[0, 0]), float(end_result_cp.iloc[0, 1]),
                ...
                SS['S16'], SS['S17']), ]
    sql_insert = 'insert into BHT_DEVICE_STATUS(DEVICECODE,TIME,DEVICE_STATUS,BROKEN_REASULT,STATUS_0,STATUS_1,STATUS_2,BROKEN_REASON)values(:1,:2,:3,:4,:5,:6,:7,:8)'
